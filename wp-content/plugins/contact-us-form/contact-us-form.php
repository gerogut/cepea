...
function showForm( $params = '' )
{
	...
	if ( empty($result) || (!empty($result) && !$this->o['hideform']) )
	{
		...
		$form .= '
			...
			<label for="cuf_sender'.$n.'" class="cuf_label">'.__('Nombre', 'cuf-lang').':</label>
		...
		$form .= '
			<label for="cuf_subject'.$n.'" class="cuf_label">'.__('Asunto', 'cuf-lang').':</label>
			<input name="cuf_subject'.$n.'" id="cuf_subject'.$n.'" size="30" value="'.$cuf_subject.'" class="cuf_field" />
			<label for="cuf_msg'.$n.'" class="cuf_label">'.__('Su Mensaje', 'cuf-lang').':</label>
